...
class Biblioteka {
    ...
    static void IspisiKnjigu(int evidencijski_broj, std::shared_ptr<Knjiga>k);
    public:
    Biblioteka () {};
...
Biblioteka::Biblioteka (const Biblioteka &b) {
    for(auto it=b.student.begin(); it!=b.student.end(); it++) {
        student.insert({it->first, std::make_shared<Student>(*(it->second))});
    }
    for(auto it=b.knjiga.begin(); it!=b.knjiga.end(); it++) {
        knjiga.insert({it->first, std::make_shared<Knjiga>(*(it->second))});
    }
}
...
Biblioteka &Biblioteka::operator =(const Biblioteka &b) {
    ...
    for(auto it=student.begin(); it!=student.end();) {
        it->second=nullptr; it=student.erase(it);
    }
    for(auto it=b.student.begin(); it!=b.student.end(); it++) {
        student.insert({it->first, std::make_shared<Student>(*it->second)}); 
    }
    for(auto it=knjiga.begin(); it!=knjiga.end();) {
        it->second=nullptr; it=knjiga.erase(it);
    }
    for(auto it=b.knjiga.begin(); it!=b.knjiga.end(); it++) {
        knjiga.insert({it->first, std::make_shared<Knjiga>(*it->second)}); 
    }
...
void Biblioteka::RegistrirajNovuKnjigu(int evidencijski_broj_knjige, std::string naslov, std::string ime_pisca, std::string zanr, int godina_izdavanja) {
    ...
    knjiga[evidencijski_broj_knjige]=std::make_shared<Knjiga>(naslov, ime_pisca, zanr, godina_izdavanja);
}
...
void Biblioteka::IspisiKnjigu (int evidencijski_broj, std::shared_ptr<Knjiga> k) {
