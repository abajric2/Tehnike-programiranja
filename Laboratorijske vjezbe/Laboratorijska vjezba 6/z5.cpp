...
auto KreirajDinamickuKopiju2D (Struktura2D matrica) {
    ...
    try {
        ...
        for(int i=0; i<matrica.size(); i++) dinamicka[i]=new std::remove_reference_t<decltype(matrica[0][0])>[matrica[i].size()];
        for(int i=0; i<matrica.size(); i++) 
           for(int j=0;j<matrica[i].size(); j++) dinamicka[i][j]=matrica[i][j];
    }
    catch(std::bad_alloc) {
        if(!dinamicka) throw;
        for(int i=0; i<matrica.size(); i++) delete[] dinamicka[i];
        delete[] dinamicka;
        throw;
    }
    return dinamicka;
...
int main ()
{
    ...
    if(dinamicka) for(int i=0; i<n; i++) delete[] dinamicka[i];
    delete[] dinamicka;
    }
    catch(std::bad_alloc) {
        std::cout<<"Nedovoljno memorije";
